...
export default function SubjectsError({
  ...
  useEffect(() => {
    console.error("Subjects section error:", error);
  }, [error]);
      ...
      <Card className="w-full max-w-lg border-destructive/30 shadow-md">
        <CardHeader className="space-y-2 text-center">
          ...
          <CardDescription className="text-sm">
            {is503
              ? "The AI backend service is currently starting up or temporarily offline. Please retry."
              : is502
              ? "Upstream AI model generation failed. Please try again."
              : "An unexpected error occurred while processing subject data."}
          </CardDescription>
        </CardHeader>
